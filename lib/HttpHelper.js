...
class HttpHelper extends SimpleClass
{
    ...
    async reAuthenticate()
    {
        ...
        try
        {
            const newLoginMethod = await Tahoma.login(username, password, linkurl, loginMethod);

            if (newLoginMethod != loginMethod)
            {
                // The login function switched methods
                Homey.ManagerSettings.set('loginMethod', newLoginMethod);
            }
            authenticated = true;
        }
        catch (error)
