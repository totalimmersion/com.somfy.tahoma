...
module.exports = class Tahoma extends Homey.SimpleClass
{
    ...
    async login(username, password, linkurl, oauthLogin, ignoreBlock = false)
    {
        ...
        try
        {
            ...
            if (!oauthLogin)
            {
                ...
                const result = await this.httpHelper.postLogin('/login', { headers: headers }, form);
            ...
            else
            {
                ...
                let formData = {
                    ...
                    username: username,
                    password: password,
                    client_id: this.Homey.env.SOMFY_OAUTH_CLIENT_ID,
                    client_secret: this.Homey.env.SOMFY_OAUTH_CLIENT_SECRET,
                };
                ...
                const result = await this.httpHelper.postLogin(this.Homey.env.SOMFY_OAUTH_URL, config, data);
    ...
    async getDeviceData()
    {
        ...
        {
            if (this.Homey.ManagerSettings.get('debugMode'))
            {
                const simData = this.Homey.ManagerSettings.get('simData');
                if (simData)
                {
                    return simData;
                }
            }
            return await this.httpHelper.get('/setup/devices');
        }
    ...
    async getDeviceSingleState(deviceUrl, state)
    {
        if (this.Homey.app.loggedIn)
        {
            return await this.httpHelper.get('/setup/devices/' + encodeURIComponent(deviceUrl) + '/states/' + encodeURIComponent(state));
        }
        throw (new Error("Not Logged in"));
    }
    ...
    async getActionGroups()
    {
        if (this.Homey.app.loggedIn)
        {
            return await this.httpHelper.get('/actionGroups');
        }
        throw (new Error("Not Logged in"));
    }
    ...
    async getDeviceStateHistory(deviceUrl, state, from, to)
    {
        if (this.Homey.app.loggedIn)
        {
            return await this.httpHelper.get('/setup/devices/' + encodeURIComponent(deviceUrl) + '/states/' + encodeURIComponent(state) + '/history/' + from + '/' + to);
        }
        throw (new Error("Not Logged in"));
    }
    ...
    async executeDeviceAction(name, deviceUrl, action, action2)
    {
        if (this.Homey.app.loggedIn)
        {
            ...
            {
                const data = {
                    label: name + ' - ' + action.name + '  - this.Homey',
                    actions: [
                    {
                        deviceURL: deviceUrl,
                        commands: [
                ...
                return await this.httpHelper.post('/exec/apply', options, data);
            }
    ...
    async executeScenario(scenarioId)
    {
        if (this.Homey.app.loggedIn)
        {
            return await this.httpHelper.post('/exec/' + scenarioId);
        }
        throw (new Error("Not Logged in"));
    }
    ...
    async cancelExecution(executionId)
    {
        ...
        {
            return await this.httpHelper.delete('/exec/current/setup/' + executionId);
        }
        throw (new Error("Not Logged in"));
    }
        ...
        if (this.Homey.app.loggedIn)
        {
            ...
            if (await this.registerEvents())
            {
                ...
                try
                {
                    ...
                    if (events === undefined)
                    {
                        throw (new Error("Undefined response for /events/" + this.registerRespons ? this.registerResponse.id : '' + "/fetch"));
                    }
    ...
    async getUserId()
    {
        return await this.httpHelper.get('/currentUserId');
    }
