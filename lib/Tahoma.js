...
module.exports = class Tahoma extends Homey.SimpleClass
{
    ...
    {
        if (!fromReAuthenticate && await this.httpHelper.checkAuthentication(false))
        {
    ...
    async eventsClearRegistered()
    {
        ...
        if (this.registerResponse && this.registerResponse.id && await this.httpHelper.checkAuthentication(false))
        {
            try
            {
                await this.httpHelper.post(`/events/${this.registerResponse.id}/unregister`);
                if (this.homey.app.infoLogEnabled)
                {
    ...
    async registerEvents()
    {
        ...
        if (!this.registerResponse || !this.registerResponse.id)
        {
            ...
            try
            {
                ...
                await this.httpHelper.checkAuthentication(true);
    ...
    async getUserId()
    {
        return this.httpHelper.get('/currentUserId', {}, false);
    }
