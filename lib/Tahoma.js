...
class Tahoma extends Homey.SimpleClass
{
    ...
    static async login(username, password, linkurl, oauthLogin, ignoreBlock = false)
    {
        ...
        if (!ignoreBlock)
        {
            ...
            this.loginAttempts20s++;
            this.loginAttempts5m++;

            // Check if the timer is running
            if (this.loginTimer20sID === null)
            {
                // Start the timer
                this.loginTimer20sID = setTimeout(() =>
                {
                    // Reset the login attempts to allow 2 more in the next time period
                    this.loginAttempts20s = 0;

                    this.loginTimer20sID = null;
                }, 20000);
            }

            if (this.loginTimer5mID === null)
            {
                // Start the timer
                this.loginTimer5mID = setTimeout(() =>
                {
                    // Reset the login attempts to allow 2 more in the next time period
                    this.loginAttempts5m = 0;

                    this.loginTimer5mID = null;
                }, 300000);
            }

            // Allow up to 2 login attempts in 20 seconds or 6 in 5 minutes
            if ((this.loginAttempts20s > 2) || (this.loginAttempts5m > 6))
            {
                throw (this.loginAttempts5m > 6 ? "Far Too many login attempts (blocked for 5 minutes)" : "Too many login attempts (blocked for 20 seconds)");
            }
        }
...
Tahoma.loginAttempts20s = 0;
Tahoma.loginAttempts5m = 0;
Tahoma.loginTimer20sID = null;
Tahoma.loginTimer5mID = null;
