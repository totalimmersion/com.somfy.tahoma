...
describe('Generic helper', function() {
	...
	describe('objAsArray', function() {
		it('transforms an object to an array but preserves the original object', function() {
			const obj = {
				obj1: {
					name: 'obj 1'
				},
			...
			expect(genericHelper.objAsArray(obj)).to.be.an('array');
			expect(genericHelper.objAsArray(obj)).to.deep.equal([{ name: 'obj 1' }, { name: 'obj 2' }]);
		...
		it('does not alter an array as input', function() {
			expect(genericHelper.objAsArray(['a', 'b', 'c'])).to.deep.equal(['a', 'b', 'c']);
		});

		it('does transform a string into an array', function() {
			expect(genericHelper.objAsArray('string')).to.deep.equal(['s', 't', 'r', 'i', 'n', 'g']);
		});

		it('throws an error when a function is passed as an argument', function() {
			const fn = function() {};
			expect(() => genericHelper.objAsArray(fn)).to.throw(TypeError);
		});
	});
