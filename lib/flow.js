...
module.exports.init = function() {
	...
	Homey.manager('flow').on('trigger.change_temperature_more_than', function(callback, args, state) {
		var conditionMet = state.measure_temperature > args.temperature;
		callback(null, conditionMet);
	});

	Homey.manager('flow').on('trigger.change_temperature_less_than', function(callback, args, state) {
		var conditionMet = state.measure_temperature < args.temperature;
		callback(null, conditionMet);
	});

	Homey.manager('flow').on('trigger.change_temperature_between', function(callback, args, state) {
		var conditionMet = state.measure_temperature > args.temperature_from && state.measure_temperature < args.temperature_to;
		callback(null, conditionMet);
	});
	...
	Homey.manager('flow').on('condition.has_temperature_more_than', function(callback, args, state) {
		var conditionMet = state.measure_temperature > args.temperature;
		callback(null, conditionMet);
	});

	Homey.manager('flow').on('condition.has_temperature_less_than', function(callback, args, state) {
		var conditionMet = state.measure_temperature < args.temperature;
		callback(null, conditionMet);
	});

	Homey.manager('flow').on('condition.has_temperature_between', function(callback, args, state) {
		var conditionMet = state.measure_temperature > args.temperature_from && state.measure_temperature < args.temperature_to;
		callback(null, conditionMet);
	});
