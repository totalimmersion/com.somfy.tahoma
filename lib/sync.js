	...
	if (lightSensorDriver) {
		...
		for(var sensorId in lightSensors) {
			if (lightSensors.hasOwnProperty(sensorId)) {
				var lightSensor = lightSensors[sensorId];
				taHoma.getDeviceStateHistory(lightSensor.data.deviceURL, 'core:LuminanceState', from, to, function(err, data) {
					//process result
					if (!err && data && data.historyValues && data.historyValues.length > 0) {
						...
						lightSensorDriver.capabilities.measure_luminance.set(lightSensor.data, mostRecentMeasurement.value, function() {});
					}
				});
			}
		}
	...
	if (temperatureSensorDriver) {
		...
		for(var sensorId in temperatureSensors) {
			if (temperatureSensors.hasOwnProperty(sensorId)) {
				var temperatureSensor = temperatureSensors[sensorId];
				taHoma.getDeviceStateHistory(temperatureSensor.data.deviceURL, 'core:TemperatureState', from, to, function(err, data) {
					//process result
					if (!err && data && data.historyValues && data.historyValues.length > 0) {
						var mostRecentMeasurement = data.historyValues[data.historyValues.length - 1];
						temperatureSensorDriver.capabilities.measure_temperature.set(temperatureSensor.data, mostRecentMeasurement.value - kelvinOffset, function() {});
					}
				});
			}
		}
