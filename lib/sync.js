...
var syncWithCloud = function() {
	...
	if (lightSensorDriver) {
		...
		for(var i in lightSensors) {
			...
			taHoma.getDeviceStateHistory(lightSensor.getDeviceUrl(), 'core:LuminanceState', from, to, function(err, data) {
				//process result
				if (!err && data && data.historyValues && data.historyValues.length > 0) {
					...
					lightSensor
						.triggerCapabilityListener('measure_luminance', mostRecentMeasurement.value);
				}
			});
		}
	}
	...
	if (temperatureSensorDriver) {
		...
		for(var i in temperatureSensors) {
			...
			taHoma.getDeviceStateHistory(temperatureSensor.getDeviceUrl(), 'core:TemperatureState', from, to, function(err, data) {
				//process result
				if (!err && data && data.historyValues && data.historyValues.length > 0) {
					var mostRecentMeasurement = data.historyValues[data.historyValues.length - 1];
					temperatureSensor
						.triggerCapabilityListener('measure_luminance', mostRecentMeasurement.value - kelvinOffset);
				}
			});
		}
	}
